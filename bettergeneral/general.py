...
class General(commands.Cog):
    ...
    @commands.command()
    async def rps(self, ctx, your_choice: RPSParser, bid: int):
        ...
        if bid > 500:
            await ctx.send("You cannot make a bid larger than 500 {currency}".format(currency=await bank.get_currency_name(getattr(channel, "guild", None))))

        else:
            pay = 0
            if outcome is True:
                if (bid > then):
                    await ctx.send("You cannot make a bid larger than your current balance!")
                elif (bid <= then):
                    now = bid
                    await bank.deposit_credits(author, now)
                    await ctx.send(
                        _("{choice} You win {author.mention}! You've won {credits} {currency}").format(
                            choice=red_choice.value, author=author, credits=now, currency=await bank.get_currency_name(getattr(channel, "guild", None))
                            )
                            )
                else:
                    await ctx.send("Contact the Bot Developer for Assistance!")
                ...
                if (bid > then):
                    ...
                    now = bid
                    multiplyer = now * 2
                    await bank.withdraw_credits(author, multiplyer)
                    await ctx.send(
                        _("{choice} You lose {author.mention}!\n You lost {credits} {currency}").format(
                            choice=red_choice.value, author=author, credits=multiplyer, currency=await bank.get_currency_name(getattr(channel, "guild", None))
                        )
                    )
                else:
                    await ctx.send("Contact the Bot Developer for Assistance")
                ...
                await ctx.send(
                    _("{choice} We're square {author.mention}! You've lost and gained nothing!").format(
    ...
    async def hug(self, ctx, user: discord.Member, intensity: int = 1):
        ...
        if intensity <= 0:
            ...
            msg = "(つ≧▽≦)つ{}⊂(´・ω・｀⊂) TOO MUCH LOVE!".format(name)
        else:
